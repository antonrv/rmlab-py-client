    ...
    * covering citysector `CityA->CityB`
    ...
    def __post_init__(self):
      ...
      if len(self.filter_class) != len(self.filter_content):
        raise ValueError(f'Mismatch in number of elements of `filter_class` and `filter_content`: `{self.filter_class}`, `{self.filter_content}`')


def make_parametric_filters_from_json(
    filename_or_list: Union[str, list]
) -> PModel:
    ...
    [
      ...
      },
      {
        ...
        "pmodel_id": "customers_choice.mnlmultiple_bl1.json"
      }
    ]
    ```
    ...
    if len(content) > 0:
      ...
      for fields in content:
        ...
        models.append(PFilter(id=model_id, 
                          filter_class=filter_class, 
                          filter_content=filter_content, 
                          pmodel_id=fields["pmodel_id"]))
      
      return PModel(id=model_id, 
                    filename=filename, 
                    hash=md5hash,
